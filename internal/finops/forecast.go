...
func projectWithDriversAsAt(db *sql.DB, desk, period string, through int) (money.Cents, string, []DriverLine, error) {
	...
	for _, ov := range overlapping {
		...
		windowDays := daysBetween(ov.start, ov.end)
...
// daysBetween is the inclusive count of calendar days from start to end,
// both "2006-01-02". Never less than one: an unparseable pair (never
