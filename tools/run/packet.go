...
//
// Bounded to packetMaxBytes regardless of what the estate holds: the packet
// is captured ONCE, at price()'s estimate time, into estimate.Packet, and
// execute() reuses that exact string rather than re-reading the database at
// call time. Two independent reads of a live estate could disagree (a
// person posts an explanation, an anomaly gets triaged, between pricing a
// run and executing it), and a worst-case bound that is only true of
// whatever the estate held a moment ago is not a bound. packetMaxBytes is
// what makes the WORST case a constant either way: packet() can never
// return more of it than that, however much the estate holds.
//
// A restricted or suspended analyst is told plainly rather than handed
// nothing silently: "You have not been given figures for this task" is a
// sentence the model can act on (ask, rather than guess), where an empty
// string reads as the packet mechanism having nothing to say.
...
// when a holds no figures-read. Every section below is independently
// omitted when it has nothing to say, and the whole block is omitted (an
// empty string) when nothing came back at all -- additive, never
// misleading, the same rule jobDescriptionBlock already holds for a name no
// role family matches.
func packet(db *sql.DB, t crew.Task, a crew.Analyst) string {
	...
	if t.Anomaly != "" {
		if an, err := anomaly.Get(db, t.Anomaly); err == nil {
			sections = append(sections, anomalySection(an))
...
func anomalySection(an anomaly.Anomaly) string {
